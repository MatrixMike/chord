...
instance Nts NewChord where
  notes (NewChord inversion b c) =
    applyMaybe (\x y -> y : x) (x : canonize (tf y) : xs) inversion
    where
      (x:y:xs) = notes c
      tf =
        case b of
...
instance Show NewChord where
  show (NewChord inversion modifier c) =
    applyMaybe
      (\x y -> x ++ "/" ++ show y)
      (applyMaybe (\x y -> x ++ show y) (show c) modifier)
      inversion
...
notesToSteps notes =
  concatMap (\(o, y) -> map (\i -> i + (o * chromaticScaleLength)) y) $
  zip [0 ..] (properGroup (\x y -> x > y) (map noteToChromaticIndex notes))

data ControlAnnotation
  = AnnotateNote
  | AnnotatePositionVertical
  | AnnotatePositionHorizontal
  | AnnotateMarking
...
tuningAndPosToNote stringTuning pos =
  canonize $ applyNTimes sharp pos stringTuning

applyNTimes f n x = iterate f x !! n

noteToChromaticIndex note =
  fromJust (findIndex (flip equiv note) chromaticScale)

extractDegrees' concept =
  map (+ (noteToChromaticIndex root')) $ map (semitones . distance root') notes'
  where
    root' = head notes'
    notes' = newNotes concept

degreeToChromaticIndex degree =
  fromJust (findIndex (flip equiv degree) degreeScale)
...
rotations =
  ...
  (\list ->
     map
       (\n -> (take (length list) . drop (length list - n)) (cycle list))
       [1 .. length list])
...
findChord inputNotes = do
  ...
  guard
    (Data.Set.isSubsetOf
       (Data.Set.fromList (uns inputNotes))
       (Data.Set.fromList notes))
...
chordTypes =
  ...
  , diminished7thChord
  , augmentedMajor7thChord
  ]

chordToNotes chord = map snd $ degrees chord

horizontalConcat str1 str2 =
  unlines $ horizontalConcat' (lines str1) (lines str2)
...
properGroup' f buf@(_:_) (x:xs) =
  if f x (last buf)
    then properGroup' f (buf ++ [x]) xs
...
hAppend w w2 =
  ...
  map
    (\lines ->
       map
         (\line -> overlay line (replicate (maximum (map length lines) + 1) ' '))
         lines) $
  (\documents ->
     map
       (\document ->
          overlay document (replicate (maximum (map length documents)) " "))
       documents)
    [lines w, lines w2]
